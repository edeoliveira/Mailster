 ...
 * ---
 * <p>
 * SWTHelper.java - A helper class that handles all the OS associated resources. Resources are
 * cached and the helper provides a <code>disposeAll()</code> method that allows to free all system
 * resources acquired till there. The class also register a JVM shutdown hook to ensure it won't be
 * forgotten.
 * 
 * @author <a href="mailto:doe_wanted@yahoo.fr">Edouard De Oliveira</a>
 * @version $Revision: 1.11 $, $Date: 2009/02/07 19:45:52 $
 */
public class SWTHelper
{
	 ...
	 */
	private final static String tempDir = MailUtilities.tempDirectory.replace(File.separatorChar, '/') + "/";

	/**
	 * The resource tracker.
	 */
	private static Map<String, Resource> resourceTracker = new HashMap<String, Resource>(20);

	/**
	 * The system font.
	 * 
	 * Note: This font should not be freed because it's allocated by the system itself.
	 */
	 ...
	 */
	private static Map<Image, HashMap<Image, Image>> imageToDecoratorMap = new HashMap<Image, HashMap<Image, Image>>();

	/**
	 * Turns the specified <code>Font</code> bold.
	 * 
	 * @param sourceFont
	 *            the source <code>Font</code> object
	 ...
	 * 
	 * @param data
	 *            the <code>FontData</code> to create the font
	 * @return Font the new font
	 */
	public static Font createFont(FontData data)
	{
		Font f = new Font(getDisplay(), data);
		resourceTracker.put(f.toString(), f);
	...
	/**
	 * Loads an image from the application's image directory and register it with a resource tracker
	 * for later disposal by the <code>disposeAll()</code> function.
	 * 
	 * @param fileName
	 *            the image file name
	 * @return the image object
	...
	/**
	 * Loads an image from the application's image directory and register it with a resource tracker
	 * for later disposal by the <code>disposeAll()</code> function if the <code>register</code>
	 * parameter is set to <code>true</code>.
	 * 
	 * @param fileName
	 *            the image file name
	 * @param register
	 *            if true image is registered in the resource tracker
	 * @return the image object
	 */
	protected static Image loadImage(String fileName, boolean register)
	{
		...
		if (o == null)
		{
			Image i = new Image(getDisplay(), MailsterSWT.class.getResourceAsStream(IMAGES_DIRECTORY + fileName));
	...
	/**
	 * Loads an image from the application's image directory, then computes the corresponding gray
	 * image and finally register the grayed image with a resource tracker for later disposal by the
	...
	/**
	 * Helper function to create a tool item. The behaviour of the toolitem is set to use a grayed
	 * image when not hovered and to use the normal image when mouse is over the toolitem. Image
	 * objects are registered in the resource tracker.
	 * 
	 * @return ToolItem
	 */
	public static ToolItem createToolItem(ToolBar parent, int type, String text, String toolTipText, String imageName,
	...
	/**
	 * Computes the gradient between two colors. Color objects are registered in the resource
	 * tracker.
	 * 
	 * @param steps
	 *            the number of steps for the color transition in the gradient
	 * @param start
	...
	public static Color[] getGradientColors(int steps, Color start, Color end)
	{
		...
		{
			colors[i] = new Color(display, end.getRed() >= start.getRed() ? colors[i - 1].getRed() + rStep : colors[i - 1]
					.getRed()
					- rStep, end.getGreen() >= start.getGreen() ? colors[i - 1].getGreen() + gStep : colors[i - 1].getGreen()
					- gStep, end.getBlue() >= start.getBlue() ? colors[i - 1].getBlue() + bStep : colors[i - 1].getBlue() - bStep);
	...
	{
		Color c = new Color(getDisplay(), r, g, b);
	...
	/**
	 * Returns the file URL of the image on the local temporary directory. If image isn't already
	 * created it creates it before.
	 * 
	 * @param fileName
	 *            the image file name
	 * @return the url if file exists or "file:///" if file couldn't be written to temporary
	 *         directory
	 */
	...
	private static boolean writeImageToTempDirectory(String fileName)
	{
		try
		{
			BufferedInputStream bin = new BufferedInputStream(SWTHelper.class.getResourceAsStream(IMAGES_DIRECTORY + fileName));
	...
	/**
	 * Try to return the current display. If no display is tied to current thread then it returns
	 * the default Display a.k.a the first created as described in Display.getDefault().
	 * 
	 ...
	 * 
	 * @param baseImage
	 *            The filename of the base image that should be decorated
	 * @param decorator
	 *            The filename of the image used to decorate the base image
	 * @param corner
	...
	{
		return decorateImage(loadImage(baseImage, true), loadImage(decorator, false), corner);
	}
	 ...
	 * @param decorator
	 *            Image The image used to decorate the base image
	 * @param corner
	 *            The corner to place decorator image
	 * @return Image The resulting decorated image
	 */
