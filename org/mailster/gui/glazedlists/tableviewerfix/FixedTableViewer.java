...
package org.mailster.gui.glazedlists.tableviewerfix;
...
public class FixedTableViewer extends FixedStructuredViewer {
    ...
    private class VirtualManager{
        ...
        public void notVisibleAdded(Object element, int index) {
            ...
            if(requiredCount > getTable().getItemCount()){
                ...
                if (requiredCount > cachedElements.length) {
     ...
     */
    private FixedTableEditorImpl tableViewerImpl;
     ...
     *            the parent control
     */
    public FixedTableViewer(Composite parent) {
     ...
     *            the table control
     */
    public FixedTableViewer(Table table) {
        this.table = table;
        hookControl(table);
...
//                      As it is possible for user code to run the event 
    ...
    private void initTableViewerImpl() {
        tableViewerImpl = new FixedTableEditorImpl(this) {
            ...
            void setSelection(StructuredSelection selection, boolean b) {
                FixedTableViewer.this.setSelection(selection, b);
            }
            ...
            if(count != list.size()){//As this is expensive skip it if all have been found
                ...
                for (int i = 0; i < virtualManager.cachedElements.length; i++) {
                    ...
                    if(virtualElements.contains(element)){
                        ...
                        update(item, null); //added
                        System.out.println("update "+ ((StoredSmtpMessage) item.getData()).getMessage().getTo());
                        indices[count++] = i;   
