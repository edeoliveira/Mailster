	...
	extends IconAndMessageDialog
{
	...
	{
		// create the list
		list = new List(parent, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.MULTI | SWT.RESIZE);
		// fill the list
		populateList(list);
	...
	private void populateList(List listToPopulate, IStatus buildingStatus, int nesting, boolean includeStatus)
	{
		...
		if (!isCoreException && t != null)
		{
			...
			for (final String line : getStackTrace(t))
				listToPopulate.add(NESTING_INDENT + StringUtilities.strip(line));
			incrementNesting = true;
		}
	...
	private java.util.List<String> getStackTrace(final Throwable t)
	{
		...
		final String[] array = StringUtilities.split(StringUtilities.replace(stackTrace, "\r\n", "\n"), "\n");
	...
	public static class StringPrintWriter
		extends PrintWriter
	{
		...
		/**
		 * <p>
		 * Since toString() returns information *about* this object, we want a separate method to
